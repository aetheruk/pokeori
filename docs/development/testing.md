...
- Events: `bun test tests/game-events.test.ts tests/event-runtime.test.ts tests/rarity-chances.test.ts` covers schedules, conflicts, temporary identities, private access, rarity thresholds/ties/inheritance, acceptance, task expiry/grace, and notification retries. `bun run test:e2e e2e/event-studio.pw.ts e2e/notification-settings.pw.ts` covers phone/desktop structured authoring and notification preferences. Event Studio's browser fixture uses an isolated transport; production dispatch and real database transactions still require deployment smoke checks.
