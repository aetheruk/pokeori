...
- Doubles sprite-grounding checks in `e2e/double-battle-layout.pw.ts` compare player/opponent shadow offsets and widths at 320px, 390px, and 1280px; visual screenshots are saved under `test-results/` for mobile and desktop review.
- Doubles: `bun test tests/double-battles.test.ts tests/battle-action-api.test.ts` covers authored PvE/PvP entry points, two-action validation, explicit targets, full spread hits, Follow Me and Ally Switch, partner-ability hooks, per-lane faint replacements and PvP perspective, the shared battle action API, and fresh lane-specific presentation events. `bunx playwright test e2e/double-battle-layout.pw.ts` checks the shared battlefield, side-by-side HUDs, player/enemy sprite depth, targeting before action, automatic second-actor/turn submission, singles-style item and move drawers, ally/spread overrides, production-paper log palette, and sequential actor/HP animation at 320px, 390px, and 1280px through a development-only fixture; it requires the isolated test MongoDB/Redis services described below. `bun run validate:data` verifies TM form/research references and the reward-free Kanto/Test trial. Production smoke should enter the trial on phone and desktop, then run one friendly and one ranked doubles battle to check queue separation, target selection, per-lane faint replacement, and four-lane visual order.
- Events: `bun test tests/game-events.test.ts tests/event-runtime.test.ts tests/rarity-chances.test.ts` covers schedules, manual enable/disable transitions, conflicts, stable identities across reactivation, private access, rarity thresholds/ties/inheritance, acceptance, task expiry/grace, paused task progress, and notification deduplication per activation. `bun run test:e2e e2e/event-studio.pw.ts e2e/notification-settings.pw.ts` covers phone/desktop structured authoring, publishing manual events switched off, repeated enable/disable actions, and notification preferences. Event Studio's browser fixture uses an isolated transport; production dispatch and real database transactions still require deployment smoke checks.
...
- `e2e/side-scroller-canvas.pw.ts` mounts the real authored Rattata Run and Pidgey Training clients with an isolated deterministic transport. It verifies that their 600x600 canvases paint changing frames and retain square responsive bounds at phone and desktop sizes.
- `e2e/artisan-balance-touch.pw.ts` mounts the real Artisan Balance dialog at mobile width. It verifies that a touch lock is recorded on pointer-down before release and that the synthetic follow-up click does not record a second lock.
- `e2e/carddex-duplicates.pw.ts` redistributes a real Carddex fixture's excess copies at phone and desktop widths, then verifies that the selected field note remains open with its reward receipt and retained-copy count after the card leaves the Duplicates result set.
