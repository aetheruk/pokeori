...
- Doubles: `bun test tests/double-battles.test.ts tests/battle-action-api.test.ts` covers authored PvE/PvP entry points, two-action validation, explicit targets, full spread hits, Follow Me and Ally Switch, partner-ability hooks, per-lane faint replacements and PvP perspective, the shared battle action API, and lane-specific scene effects. `bunx playwright test e2e/double-battle-layout.pw.ts` checks the shared battlefield, side-by-side HUDs, player/enemy sprite depth, sequential controls, and target selection at 320px, 390px, and 1280px through a development-only fixture; it requires the isolated test MongoDB/Redis services described below. `bun run validate:data` verifies TM form/research references and the reward-free Kanto/Test trial. Production smoke should enter the trial on phone and desktop, then run one friendly and one ranked doubles battle to check queue separation, target selection, per-lane faint replacement, and four-lane visual order.
