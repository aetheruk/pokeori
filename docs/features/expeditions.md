...
- The eight Kanto Gym Badge Chronicles are fixed, secret, non-branching eight-step routes. Each uses five first-person narrative tasks, two battles, one signature mini-game, a leader-specific fixed team, and `canFail: false`; normal Chronicle reward suppression keeps the player from retaining activity rewards or caught Pokemon from another person's memory.
