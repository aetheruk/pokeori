...
- Internal activities remain resolvable from expedition paths when authored in the `Secret` category, while Explore excludes that category entirely. Koga's Gym Trainer battles, invisible mazes, reward scene, and all Safari catch, Field Observation, and Strength-cache steps follow this established pattern. Locations and games can additionally set `expeditionOnly` as a defensive visibility marker; Safari catches use both protections.
