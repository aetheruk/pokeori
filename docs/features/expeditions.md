...
- Expedition entry fees can use consumed currency criteria. The Safari Zone uses one 500-Pokédollar Grand Expedition with five lives and 30 run-scoped Safari Balls. Its generated path spans 36 steps across Central, East, West, and North with exactly 10 Safari catches, eight expedition-only Field Research surveys, four to six weighted observation tasks, area-specific flavor, and occasional Rocket poacher battles. Observation variants are weighted 70/25/5 for 2/10/20 Pokémon Research XP, while item tasks draw non-guaranteed materials, balls, gems, currency, extra Safari Balls, and rare one-time finds. Flavor tasks grant a small 20-50 Researching XP roll. Once HM04 Strength is owned, the final catch uses a dedicated five-species Grand Finale habitat. Completion grants only the authored XP rewards, with an inverse-ownership HM04 Strength reward on the first completed run. The old four Safari expedition definitions remain resolvable only for active or claimable legacy runs and cannot be started again.
