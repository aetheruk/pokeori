...
- Evolution Catalysts are guaranteed Balance crafts at Artisan 5, 20, and 40. The low, mid, and high catalysts cost 25/300/500 Crystals plus 10 XS, 25 S, or 50 M PokePowder respectively, use palette-swapped Scarlet/Violet Max Revive sprites, and are consumed by pure level-based Pokémon evolutions. The first Evolution Catalyst and access to all three recipes arrive with the Jungle Binder reward from Bug Catcher Benny; an optional Viridian Forest conversation with Benny explains the item through natural forest and Pokémon-growth imagery without gating recipe access. Max Revive is also registered as a Battling 60 full-heal revive item.
- Candy Bags are Balance crafts unlocked by the Fuchsia `Build in bulk` task. All ten bags unlock at the same Artisan levels as their matching PokePowder recipes (2 through 90 in ten-level steps), cost 10 of the matching candy, offer x2 bulk crafting, and produce one bag on Good or Perfect quality. Bad crafts fail without consuming the candy. Each bag sets a Pokémon directly to the top level of its tier rather than adding ten levels, from XS Candy Bag at level 10 through XL Candy EX Bag at level 100.
- Regular candies and Candy Bags from the M tier onward have a chance to fail to raise or set a Pokémon's level: 90% for levels 41-50, 80% for 51-60, 70% for 61-70, 60% for 71-80, 50% for 81-90, and 25% for 91-100. Failed uses still consume the candy or bag.
...
Special Pokemon item rolls live in `SPECIAL_POKEMON_DROPS`, keyed directly by Pokemon form ID. Each configured drop rolls independently from primary material type selection, quantity rolls, and other special drops; entries can use normal reward `requirements`, and `guaranteed: true` drops always create an active Field Observation drop outside the primary-material cap. Pikachu has an unconditional 1% Light Ball roll. Base and Galarian Farfetch'd have a 1% Leek roll; Cubone, Marowak, and Alolan Marowak have a 1% Thick Club roll; and Ditto has separate independent 1% Quick Powder and Metal Powder rolls. Chansey gains independent 1% Lucky Egg and Lucky Punch rolls after Rare Item Rumours is completed; these no longer roll from unrelated Safari catches. Entei, Raikou, and Suicune currently map to Token of Fire, Token of Thunder, and Token of Water. Combining each element's Mark, Proof, and Token through the guaranteed beast-stone random task creates a matching Lifeless key item, which unlocks a one-time Artisan 50 recipe for the matching Concentrated key item. The three bird-branch fusion tasks likewise unlock their matching one-time Concentrated Branch recipe at Artisan 50.
...
- Artisan recipe cards and craft-result drawers use the dedicated warm `artisan-workshop.avif` background rather than the unrelated Cerulean bike shop scene.
- All dried-dye recipes cost 3 matching base Dye, use the Mix craft check, and retain their x3 bulk action.
- The TCG category unlocks after the Basic Training “My Very Own Set” task. Foil Pack unlocks at Artisan 15, uses Scatter, and costs 2 T1 Metal Scrap, producing 0/3/5 on Bad/Good/Perfect quality. Its x3 bulk action unlocks at Artisan 20 through the shared five-level bulk rule. Its Artisan prompt tells the player to add Scrap Metal to the machine, and its Empty Foil Pack output uses a clean logo-free wrapper sprite. Base Set, Jungle, Fossil, Base Set 2, Team Rocket, and Gym Heroes Booster Packs unlock at Artisan 22, use Balance, and cost one Empty Foil Pack plus their matching Dried Purple, Green, Yellow, Red, Black, or Blue Dye respectively. Team Rocket additionally stays hidden until the player owns its binder, while Gym Heroes requires all 132 Gym Heroes cards. Every set-pack recipe uses the shared printer-calibration description, produces 0/1/2 on Bad/Good/Perfect quality, and consumes materials on Bad.
...
- Recipe categories are Materials, Balls, Lures, Held, Items, Special, and TCG. Special is reserved for quest and task recipes.
- `craft-tiny-bug-armour` is a Special recipe unlocked by the completed Bug God offering chain plus the Jungle binder, costs 3 Bug material plus 2 XS PokePowder, uses Scatter, requires Good quality, consumes materials on Bad failure, produces `tiny-bug-armour` for the repeatable Bug God hand-in, and shows an x10 bulk craft once the player is 5 Artisan levels above the recipe. The hand-in and recipe remain available for one first hand-in when the player already has all 64 unique Jungle cards, then close after that hand-in; otherwise the hand-in pays a Jungle booster pack until the set is complete and then pays 2 Chitin Fragments instead. Completing that first full-set hand-in unlocks the one-time Bug Out wrap-up task for 500 catching XP.
- `craft-day-care-clay-brick` is a Special recipe unlocked during the post-Thunder Badge Day Care building repairs, costs 1 Soft Clay (`terra-dust-t1`), uses Mix, requires Good quality, consumes clay on Bad for no brick, produces 1 Kiln-Fired Brick on Good and 2 on Perfect, has no bulk craft, and the output sells for 10 Pokédollars each.
