...
- The owned Pokemon detail modal capitalizes revealed natures and, once the Nature Module is owned, marks nature-boosted stats with a charcoal up arrow and nature-reduced stats with a red down arrow. Evolution previews capitalize the revealed evolved Pokemon name.
...
- Move-based evolution requires the specific move in the Pokemon's saved assigned moves. Owning its TM or staging an unsaved loadout is insufficient. The detail panel names the required move under Battle Moves and shows the evolution action once all conditions are met; the server independently validates the persisted loadout. These branches no longer require or consume Charged TMs, do not consume the equipped move/TM, and do not require another level-up or an Evolution Catalyst. Existing form and other conditions still apply. Evolution data and its generator use the battle catalog's move slugs.
...
- MoveDex exposes Get missing TMs below the Known moves filters. It checks the signed-in trainer's earned research rewards, restores missing eligible TMs, and refreshes inventory without leaving the page. The button disables while checking and reports recovered rewards, no missing rewards, or a failure.
