...
`/api/game/sync` accepts a route-derived scope so each game surface only fetches the normalized collections it renders. Unrequested fields are omitted rather than populated with empty placeholders. The client keeps each scope in its own SWR cache, deduplicates foreground requests, and performs a low-frequency background refresh. Explore requests the scoped slim Pokemon payload because Explore requirement and selection UI need those fields, and it also requests `shopPurchases` because its shop drawers render limited-stock availability. Client code should read normalized data from `gameData` rather than expecting `user.inventory`, `user.pokedex`, `user.completedTasks`, `user.tcg`, or result maps under `user.stats`.
