...
- When the battle reaches a terminal result, it automatically claims the result and opens the shared rewards/results screen without a separate winner-card animation. Show Results remains available as a fallback, and the result screen stays open until the player continues. Currency-only results initialize directly on the summary step to avoid a closed-dialog flash during the handoff.
