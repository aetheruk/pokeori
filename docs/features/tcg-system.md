...
- Once the final winner animation finishes, the battle automatically claims the result and opens the shared rewards/results screen. The visible Show Results action remains as a fallback, and the winner overlay is removed when the shared result opens so it cannot cover the rewards screen.
