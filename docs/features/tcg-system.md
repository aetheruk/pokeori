...
- Basic Training content is authored in Kanto Underground and uses the TCG Quiz, TCG Battle, and Art Academy engines. TCG battles remain Pokemon-only 15-card decks; Trainer cards are intentionally unsupported in the current battle rules. The Baby, Champions, and Masters formats use total deck-cost caps of 30, 55, and 85, and Kanto Underground battles use Base Rules. The Pewter School lessons explain Trainer-page Auto Fill, CardDex manual card additions, format costs, card and attack turn unlocks, and charge pacing. After the Fire, Water, and Grass trials, the `Don't Get Distracted` task congratulates the player, explains that battles do not generate Crystals, and unlocks the first-card Art Academy game after the player acknowledges that making cards, packs, and sets is the profitable TCG work.
- Kanto Underground Fire, Water, and Grass trials and rematches use Charizard, Blastoise, and Venusaur icons respectively. Their authored opponent decks are validated as legal 15-card Baby decks before battle start, including the format cost cap.
