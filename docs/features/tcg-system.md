...
The Carddex preserves an artwork-first index and organizes unlocked expansions as a newest-first series and binder shelf. Scope and filters persist in the URL, search covers card names, collector numbers, set names, and series without leaving the selected shelf, and mobile places the complete filter set in a full-height responsive field note. Multi-set results retain set dividers. Owned-card notes pair artwork with compact metadata, previous/next browsing, collection quantity, rarity, duplicate redistribution, and battle-deck actions; touch layouts stack the same information. After duplicate redistribution, the selected note stays open through the collection refresh so its receipt and retained-copy count remain visible even when the card leaves the active Duplicates filter.
