...
TCG catalog responses are versioned with the application release and privately returned without shared caching because they include `ownedTotal`, the authenticated trainer's ownership count across the complete matching query. That count remains stable as cursor pages append. Search, ownership, supertype, Pokémon type, normalized rarity, and stable sort rules are applied before pagination, while uncollected cards use discovery-safe visual and accessible labels.

The Carddex preserves an artwork-first index and organizes unlocked expansions as newest-first series and set shelves. The shelves use representative series logos and canonical set symbols instead of generic binder artwork. Scope and filters persist in the URL, search covers card names, collector numbers, set names, and series without leaving the selected shelf, and mobile groups collection scope separately from card-detail filters in a full-height responsive field note. Multi-set results retain symbol-led set dividers. Owned-card notes pair artwork with compact metadata, previous/next browsing, collection quantity, rarity, duplicate redistribution, and battle-deck actions; touch layouts stack the same information. After duplicate redistribution, the selected note stays open through the collection refresh so its receipt and retained-copy count remain visible even when the card leaves the active Duplicates filter.

## TCG Battle
- Decks require exactly 15 unique owned Pokemon cards with HP, type, image, and attacks.
- The Trainer deck editor shows the actual card artwork for selected cards and displays the current/max format cost. CardDex battle-deck actions show the card's cost to add plus the current card-count and cost totals for each format.
...
- Kanto Underground adds a free, unranked Base Champions PVP table after the TCG battle wrap-up. It requires the `deck-box`, is unavailable in Kid Mode, and revalidates each collector's saved Base Champions deck against ownership, series, legality, card count, and the Champions cost cap before matchmaking can create a battle.
- TCG PVP supports six-digit friendly lobbies and an unranked Quick Match queue. The Explore card fixes the format and deck level; players cannot negotiate or override them in the lobby.
- PVP stores one authoritative shared battle under a match-scoped Redis key and returns a redacted viewer-relative copy to each client. The local collector is always rendered at the bottom, the opponent at the top, private hands and unused opponent deck cards are never sent, and opening arrangements remain hidden until both collectors are ready.
- A random collector takes the first turn, then actions alternate. Setup, turns, and required promotions use two-minute server deadlines. If only one opening board is ready at setup expiry that collector wins; if neither is ready the match is a no-contest. Later action expiry awards the match to the waiting collector, and surrender is available from the battle header.
- PVP has no stake, payout, reward, or ranking. Decisive outcomes record one win and one loss against the PVP Explore entry; exact draws and setup no-contests record neither. All outcomes still flow through the standard TCG winner animation and shared results screen.
...
- Once the final winner animation finishes, the battle automatically claims the result and opens the shared rewards/results screen. The visible Show Results action remains as a fallback, and the winner overlay is removed when the shared result opens so it cannot cover the rewards screen. Currency-only results initialize directly on the summary step to avoid a closed-dialog flash during the handoff.
...
- Basic Training content is authored in Kanto Underground and uses the TCG Inspection, TCG Battle, and Art Academy engines. Gideon recruits the player after mistaking a Promo binder for membership paperwork, while Mina explains the organization’s card-for-crystals operation and treats the talking pit as routine office infrastructure. The 500-Crystal contribution remains a one-time sink; duplicate redistribution returns cards to HQ for repacking and compensates the player in Pokédollars rather than generating crystals directly. Its one-time Pikachu card-design lesson uses a 30% Art Academy passing threshold. The later “My Very Own Set” lesson follows the battle wrap-up without requiring completed Base or Jungle collections, awards the Base Set 2 binder, and unlocks foil-plus-dried-dye recipes for Base, Jungle, Fossil, and Base Set 2 packs; the matching Team Rocket recipe additionally requires its binder.
- TCG battles remain Pokemon-only 15-card decks; Trainer cards are intentionally unsupported in the current battle rules. Baby, Champions, and Masters use total deck-cost caps of 30, 55, and 85, and Kanto Underground uses Base Rules. The rewritten Pewter School lessons retain Trainer-page Auto Fill, CardDex manual additions, format costs, card/attack unlock turns, energy pacing, effects, retreating, knockouts, and promotion across shorter dialogue steps.
- Passing the Pewter quiz unlocks `Lost-and-Found Practice`, a one-time 250-Pokédollar tutorial battle against a legal mixed-type Base Baby deck containing fifteen 1-cost Basic cards. Winning introduces the serial slacker arc: Cal’s Fire demonstration, Marina’s Water quality audit, and Fern’s Grass captive-audience tournament each receive a conversation task before their existing one-time battle. The elemental IDs, 1,000-Pokédollar rewards, decks, and recorded wins remain unchanged. New story tasks include an inverse battle-wrap-up requirement so completed players do not receive retroactive unfinished conversations.
- The tutorial, Fire, Water, and Grass battles use Rattata, Charizard, Blastoise, and Venusaur icons respectively; the three post-training rematches retain the elemental icons and 350-Pokédollar payouts. The Celadon Game Corner tables use the male TCG Maniac sprite for the low-stakes table and the female sprite for the high-stakes table. Every authored opponent deck is validated as a legal 15-card deck in its authored format before battle start, including the format cost cap.
- TCG Battle Explore drawers show Wins and Losses plus the authored deck level and card format, so players can distinguish Baby, Champions, or Masters rules and sets such as Base or Gym before starting.
- Carddex shelves present the newest unlocked series and expansions first for fast collection access. Trainer deck-series selectors retain chronological release order rather than alphabetical order.
- Trainer Auto Fill builds fifteen cards with a format-aware strength selection under the Baby, Champions, or Masters cost cap, so higher formats use their additional budget when stronger owned cards are available.
- Card Quality Control uses a 30-second manual study phase with a previous/next card carousel and a Ready action. The study timer is separate from the question timer, which starts when Ready is pressed; each session gives two lives, and incorrect answers remove one life. The player must answer three questions correctly, with no question-count cap: generation continues until the target is reached, both lives are lost, or time expires. Question generation avoids immediately repeating the same card, question type, and answer. Session expiry includes both study and question windows, while answer-count validation and rewards remain server-authoritative.
