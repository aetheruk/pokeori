...
- Pachinko physics always run in the authored board coordinate space; CSS scales only the rendered canvas. Arrow positions are clamped inside the physical walls, bucket sensors sit behind their rails so a grazing ball cannot claim a prize, and any ball still unsettled after 30 seconds is recorded as a miss. These rules keep odds and settlement behavior stable across viewport sizes.
- Celadon Rocket Pachinko has three rewarding buckets: its two 15-token `Prize` buckets sit at the board edges, while its narrow centre `Jackpot` pays 50. High Stakes uses the same geometry with 75/250-token prizes. The lower slanted guides remain below the final peg row so a ball cannot wedge against the diagonal lines.
