...
- Surf uses a dedicated portrait course that fills the mobile viewport and becomes a framed 9:16 stage on desktop. The player drags anywhere in the course to steer side to side, with Arrow/A/D keyboard controls as a desktop alternative. Configurations can supply different player art: the endless test uses a custom hand-painted rear-view Lapras, while the finite test uses a transparent rear-view wooden motorboat and completes at 800 points. The player stays near the bottom with a buoyant bob, steering lean, diffuse mirrored wake plumes, and soft spray while speed-linked scene layers sell forward travel: the painted sea is duplicated below its authored horizon and visibly expands from the vanishing point through a seamless crossfade, moving foreground detail substantially faster than distant water while existing surface ripples add finer movement. Separately authored high and low cloud bands share a calm lateral drift with restrained depth expansion, and the island/cliff layer visibly approaches around the horizon behind soft mist. The complete wake shares the player's centred steering transform and uses one keyframe-owned horizontal centring transform so its animation cannot double-shift left. The initial countdown introduces drag steering without retaining a persistent bottom-screen hint, and the standard open-door leave control sits at the top right. All decorative motion respects reduced-motion preferences. The original still Lapras sprite is retained beside the active asset as `lapras-rear-flat.png` for fallback. Obstacles and endless collectible rewards emerge at the painted sea horizon (28% stage depth) at 12% scale, fade and sharpen into view, then accelerate toward the observer along an ease-in depth curve while expanding and spreading with perspective; extending the course below the viewport preserves the established arrival and collision cadence while providing substantially more look-ahead. Multiply contact shadows and a sprite-masked cyan sheen visually seat hazards in the water without reducing collision readability. Authored settings control speed, maximum speed, acceleration, steering speed, difficulty, obstacle cadence and weighting, layered scene art, finite score targets, and the shared endless reward model.
