...
- Materials, Field Observation berry pools, broken balls, global item events, and route-authored `settings.itemDrops` are pre-rolled at round start and appear as tappable drops in the observation panel. Global item events include Escape Rope, Explorer 20 Repel, Researcher's Journal items that grant Researching XP when used, and a 10% Research Kit collectible at Researcher level 35+. A successful Research Kit roll is protected from the normal six-drop limit. Taps are validated server-side against the Redis session, generated drop list, and visible time window.
