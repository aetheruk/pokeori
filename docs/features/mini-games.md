...
Run and Flap render those fixed-step snapshots with compositor transforms, including their repeat-x parallax tracks, so high-refresh displays interpolate motion without adding simulation ticks or changing server replay. Sprite-sheet frame discovery reads image dimensions only; collision no longer performs a main-thread pixel-mask readback. Brief foreground frame stalls can catch up through a bounded 250ms accumulator rather than permanently slowing the run.
