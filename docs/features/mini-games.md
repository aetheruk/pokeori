...
- Surf uses a dedicated portrait course that fills the mobile viewport and becomes a framed 9:16 stage on desktop. The player drags anywhere in the course to steer side to side, with Arrow/A/D keyboard controls as a desktop alternative. A custom hand-painted rear-view Lapras stays near the bottom with a buoyant bob, steering lean, diffuse mirrored wake plumes, and soft spray while clearly moving cloud banks and layered water sell forward travel; the complete wake shares Lapras's centred steering transform, begins beneath the tail, and remains visually distinct from the backdrop's fixed sunlight reflection at every course position. The initial countdown introduces drag steering without retaining a persistent bottom-screen hint, and the standard open-door leave control sits at the top right. All decorative motion respects reduced-motion preferences. The original still sprite is retained beside the active asset as `lapras-rear-flat.png` for fallback. Obstacles and collectible rewards travel from the horizon toward the player with perspective scaling. Authored settings control speed, maximum speed, acceleration, steering speed, difficulty, obstacle cadence and weighting, scene art, finite score targets, and the shared endless reward model.
