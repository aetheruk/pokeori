...
- Surf uses a dedicated portrait course that fills the mobile viewport and becomes a framed 9:16 stage on desktop. The player drags anywhere in the course to steer side to side, with Arrow/A/D keyboard controls as a desktop alternative. A custom hand-painted rear-view Lapras stays near the bottom with a responsive wake, spray, and steering lean while subtle sky drift and layered moving water sell forward travel; all decorative motion respects reduced-motion preferences. Obstacles and collectible rewards travel from the horizon toward the player with perspective scaling. Authored settings control speed, maximum speed, acceleration, steering speed, difficulty, obstacle cadence and weighting, scene art, finite score targets, and the shared endless reward model.
