...
- Battle Bets is a custom High Stakes Room mini-game session under `battle-bets:{userId}` rather than the generic `game:{userId}` completion runtime. Completing its unlock task consumes 100 Fun Tokens once and seeds a 100-token virtual pot. Opening later books never debits the wallet; only a virtual pot can be lost. Restoring the same one-hour session resumes its inspection, live battle, or result state, and expiry forfeits the unclaimed virtual pot.
...
- Selecting a side commits the entire virtual pot and starts a persisted live spectator battle. The server chooses both Advanced AI actions and resolves one shared-engine turn per request; the client waits for `useBattleManager` to finish the normal battle animation queue before requesting the next turn. This uses the normal battle scene, team HUD, sprites, HP animation, faint/replacement sequences, effects, and battle log, with a compact read-only status strip where player controls normally sit.
- A winning result remains virtual until the player explicitly cashes out or rolls the full payout into a newly generated matchup. Losses pay zero. The result is shown in a fixed, safe-area-aware layer after the final battle animation so cash-out and rollover controls remain reachable on small mobile screens. Start, wager, live turns, rollover, and cash-out transitions use rate limits, action locks, and idempotent result keys where a duplicate could advance or grant state twice.
