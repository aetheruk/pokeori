...
Manual state is stored in the existing definition JSON (`timingMode`, `enabled`), so no collection migration is required. Server-managed start/end timestamps preserve indexed runtime queries and claim expiry. Enabled manual events use an internal open-ended upper bound; player and admin views say “Until switched off” instead of exposing that date. Existing definitions without a timing mode remain scheduled.
