...
Battle state is a semantic exception to the routine interface palette: live/full HP and recovery may use health green (`game-health`), while the rest of the interface remains charcoal-led. Battle stance colors are fixed as Power = clay/red, Speed = ochre/yellow, and Tech = stance blue.
...
- Drawers use gesture dismissal and keep their scenic headers free of redundant close buttons. Their category chip sits below the centered icon as a glass orb treatment, with a centered icon/text row, tight line-height, and comfortable horizontal inset. Drawer overview copy is left-aligned without an accent rail, rules and stats use unboxed ledger rows with bare icons, and requirements/reward carousels use single unboxed journal slides with bare item icons and no second wrapper card. Location inhabitants, fishing encounters, and wild-battle encounters use sprite-only strips: unseen sprites are black silhouettes, seen-but-uncaught sprites are 50% opacity, and caught sprites are full opacity. Level details belong in the Rules ledger as one combined Level Range.
- Battle move lists make the full move row the selection target; move details are opened through a frameless information icon. Battle item lists and result-screen rewards use the same unboxed ledger-row treatment as game-info stats, with separators providing grouping instead of white card wrappers.
