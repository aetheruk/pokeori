...
- Explore list cards keep the activity artwork on the left, remove redundant right-side type accessories, and right-align the title and status. When content provides a background, use it as a low-opacity scenic layer beneath a directional raised-paper fade so the scene remains visible on the art side while titles and statuses stay readable. Repeatable tasks use a small repeat badge at the bottom center of their icon. When a card groups multiple activities, its actions stay right-aligned and use clean two-part buttons with semantic activity-colored icon wells; every activity icon sits directly in its colored well without an extra icon orb.
