...
- Battle move lists make the full type-colored move bar the selection target. A long hold, right click, or focused I key opens the complete move details. Battle item lists and result-screen rewards use the same unboxed ledger-row treatment as game-info stats, with separators providing grouping instead of white card wrappers.
