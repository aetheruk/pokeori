...
- Explore list cards keep the activity artwork on the left, remove redundant right-side type accessories, and right-align the title and status. When content provides a background, use it as a low-opacity scenic layer beneath a raised-paper overlay so the card keeps readable field-journal contrast. Repeatable tasks use a small repeat badge at the bottom center of their icon. When a card groups multiple activities, its actions stay right-aligned and use clean two-part buttons with semantic activity-colored icon wells; location actions use the colored well directly without an extra icon orb.
