...
- Region, area, and trainer-section selectors use longer scenic cards with a directional raised-paper fade and right-aligned copy; visible section titles belong in the card, not in a redundant selector banner. Pokémon Box cards may carry the caught Pokémon's background in a small inset scene chip behind the sprite, with a soft shadow for contrast.
