...
The production browser sample used fresh 1280px Chromium contexts and an observation window ending 800 ms after hydration. Pokémon/inventory fixtures were empty, and Run was observed during its countdown. Explore, box, inventory, and admin script totals were roughly unchanged. Script counts do not establish reductions in authenticated sync, Server Action, or RSC requests. No populated-list scrolling result is established by this sample.
...
| P3 | Mutation-domain invalidation and scoped snapshot tests | Browser request counts after representative reward mutations and route revisits, including zero unrelated-scope revalidation and fresh affected data. |
| P4–P5 | Compatible Turbopack analysis; observed Run split; item presentation index parity | Compressed initial-route budgets and an enforced regression threshold; asynchronous graph totals remain a different measure. |
...
| P7 | Pinned validation/integration workflows and local real endpoint tests | Successful checks on the exact release merge candidate and required branch checks enabled remotely. A workflow file alone does not enforce merging policy. |
...
| P9 | Sync authentication/data/serialization/total timing, response bytes, existing battle timing; opt-in economy duration/attempt/retry/replay/error and lock acquisition/contention/release events | Production aggregation, host/pool/index/eviction measurements. Lock durations measure Redis acquisition/release calls; no blocking wait queue exists. |
...
| P11 | Intent prefetch; separate mobile/desktop tests each load one auth image | Authenticated prefetch/RSC request counts; populated multi-page list DOM/heap/long-task/scroll measurements before considering virtualization. |
| P12 | Lossless compressed receipts, semantic aliases, replay tests, report/compaction script | Real receipt volume/age/response-size distribution and growth trend. Preserve permanent replay identities when compacting. |
