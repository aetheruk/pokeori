...
async function syncFormSprites(form) {
  ...
  for (const definition of getSpriteDefinitions(form.formId)) {
    if (await pathExists(definition.source)) {
      ...
      definition.required &&
      (definition.key.startsWith('home.') || definition.key.startsWith('genV.front.') || definition.key.startsWith('genV.back.')) &&
      (await pathExists(definition.output))
    ) {
      // Some newer forms and event costumes have no canonical asset in the
      // bundled PokeAPI source. Keep locally generated project assets in the
      // manifest and out of the missing report on subsequent syncs.
      setManifestPath(entry, definition.key, publicPath(definition.output))
      generatedSprites.push(definition.output)
    } else if (definition.required) {
      missing.push({ ...form, sprite: definition.key })
    }
  }

  return { form, entry, missing, copied, requiredCopied, generatedSprites, optionalFound, skippedExisting }
}
...
async function syncSprites(forms, args) {
  ...
  const generatedSprites = results.flatMap((result) => result.generatedSprites)
  ...
  return { sprites, missing, copied, requiredCopied, generatedSprites, optionalFound, skippedExisting }
}
...
async function writeMissingDoc({ forms, missing, requiredCopied, generatedSprites, optionalFound, skippedExisting }) {
...
- Locally generated sprites reused: ${generatedSprites.length}
