...
export async function completeCurrentUserExpeditionTaskStep(
  ...
  // This shortcut is only for replaying a task already earned on an earlier run.
  // First-time and repeatable tasks must use completeTask's cost/reward checks.
  const task = tasks.find((entry) => entry.id === taskId)
  const completed = await getUserCompletedTasksMap(payload, user.id)
  if (!task || task.repeatable || !completed[taskId]) {
    return { success: false, message: 'Complete this task before replaying its expedition step' }
  }
