...
export async function hatchEgg(eggId: string, clientActionId: string) {
  ...
  try {
  const response = await runEconomyAction<any>(
    { userId: user.id, action: 'hatch-egg', requestId: clientActionId },
    async ({ payload, req }) => {
  const freshUser = await payload.findByID({ collection: 'users', id: user.id, req })
  const egg = await payload.findByID({ collection: 'user-eggs' as any, id: eggId, depth: 0, req }).catch(() => null) as any
  ...
  const crystals = (freshUser.currency as any)?.crystals || 0
  ...
  const result = await getEggHatchOutcome(payload as any, freshUser, eggRarity, req)
  if (!result) return { success: false, message: 'No eligible Pokemon are available for this egg yet.' }

  await payload.update({ collection: 'users', id: user.id, data: { currency: { ...freshUser.currency, crystals: crystals - DAY_CARE_EGG_HATCH_COST_CRYSTALS } }, req })
  ...
  ], { source: 'day-care-egg-hatch', skipDropChance: true, payload, req })
  await payload.update({ collection: 'user-eggs' as any, id: egg.id, data: { status: 'hatched', hatchPoolId: result.poolId }, req })
  return { success: true, summary, eggRarity, message: `${summary.pokemon[0]?.name || 'A Pokemon'} hatched from the egg!` }
    },
  )
