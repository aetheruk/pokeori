...
export function BattleControlRegion({
  ...
  return (
    <div className="game-battle-control-region game-paper-first relative flex min-h-0 flex-[38] flex-col border-t border-game-border bg-game-surface-raised text-game-ink">
      {panel ? (
        <section className="game-battle-section relative flex min-h-0 flex-1 flex-col overflow-hidden" data-type={panel === 'stance' ? stanceType?.toLowerCase() : undefined} aria-label={`${titles[panel]} choices`}>
          {panel === 'stance' && <BattleTypeAtmosphere key={stanceType?.toLowerCase() ?? 'normal'} type={stanceType ?? 'normal'} />}
          ...
          <div className="game-battle-section-body relative z-10 min-h-0 flex-1 overflow-y-auto overscroll-contain px-3 py-3 sm:px-4">
            {children}
          </div>
        </section>
      ) : (
