...
import { replayCaptureSettlement, runCaptureSettlement } from './capture-settlement'
...
export async function getQuizQuestion(
  ...
  try {
    ...
    if (!user) return null
    const state = (await redis.get(`encounter:${user.id}`)) as EncounterState | null
    if (!state) return null
    return buildEncounterQuizQuestion({
...
export async function useEncounterItem(itemId: string, clientActionId?: string) {
  try {
    const user = await getUser()
    if (!user) return { success: false, error: 'Unauthorized' }

    if (typeof clientActionId !== 'string' || !/^[a-zA-Z0-9_-]{1,80}$/.test(clientActionId)) return { success: false, error: 'Invalid item action' }
    const replay = await replayCaptureSettlement(user.id, clientActionId, 'item')
    ...
    try {
      ...
      if (state.encounterMode === 'safari') {
        return {
          success: false,
          error:
            'Safari encounters use Oran Berries and Tamato Berries instead of encounter items.',
        }
      }
      ...
      if (itemId === 'escape-rope') {
        expeditionProgress = await failEncounter(user, state, undefined, { payload, req, redis })
        fled = true
      ...
      // 5. Chaos Stone
        ...
        if (Math.random() < 0.5) {
          ...
          newRate = Math.max(0, state.currentCatchRate - 3 * stageValue)
          message = 'Chaos Stone reduced catch rate by three stages!'
        }
      }

      // Key encounter tools such as Chaos Stone persist after use.
