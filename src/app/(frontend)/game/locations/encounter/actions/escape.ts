...
export async function runAway() {
  ...
  if (!rateLimit.allowed) {
    return {
      success: false,
      message: 'Too many escape attempts. Please wait a moment.',
    }
  }
  ...
  if (!escapeLock.acquired) {
    return {
      success: false,
      message: 'Another escape action is already being processed',
    }
  }
  ...
  try {
    ...
    if (!state) {
      const recentRunAway = await getIdempotentResult<any>(
        `encounter:runaway:last:${user.id}`,
      )
      if (recentRunAway) {
...
export async function attemptAbilityEscape() {
  ...
  try {
    ...
    const escapeChance = getAbilityEscapeChance({
      ...
      sourceFormId:
        state.activeAbilitySourceFormId || (activePoke.docs[0] as any)?.formId,
    ...
    if (state.abilityEscapeAttempted) {
      return {
        success: false,
        message: 'You already tried to use this ability!',
      }
    }
    ...
    if (escaped) {
      const expeditionProgress = await performRunAway(
        user,
        state,
        payload,
        true,
...
export async function researchEscape() {
  ...
  try {
    ...
    if (researchLevel < 3) {
      return {
        success: false,
        message: 'Research level too low for free escape',
      }
    }
