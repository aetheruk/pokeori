import { BattleConfig } from '../../types'

export const pvpBattles: BattleConfig[] = [
  {
    id: 'pvp_friendly_test',
    name: 'Friend Battle',
    description:
      'Challenge a friend to a battle, with nothing at stake! | [3 Pokemon auto set to level 50]',
    category: 'Pokemon Center',
    subCategory: 'Battles',
    icon: { type: 'item', id: 'vs-seeker' },
    background: '/backgrounds/friend-stadium.avif',
    requirements: [
      {
        type: 'item_owned',
        targetId: 'vs-seeker',
      },
      {
        type: 'kid_mode',
        inverse: true,
      },
    ],
    enemyTeam: [],
    rewards: [],
    maxPokemon: 3,
    levelCap: 50,
    enemyAttackTelegraphChance: 2,
    pvp: true,
    pvp_type: 'friendly',
  },
  {
    id: 'pvp_ranked_test',
    name: 'Crystal League',
    description: 'Take part in ranked battles in the Crystal League',
    category: 'Pokemon Center',
    subCategory: 'Battles',
    icon: { type: 'item', id: 'vs-seeker' },
    background: '/backgrounds/crystal-stadium.avif',
    requirements: [
      {
        type: 'item_owned',
        targetId: 'vs-seeker',
      },
      {
        type: 'kid_mode',
        inverse: true,
      },
    ],
    enemyTeam: [],
    rewards: [
      { type: 'currency', targetId: 'pokedollars', quantity: 100 },
      { type: 'currency', targetId: 'battle-points', quantity: 5 },
    ],
    maxPokemon: 3,
    levelCap: 20,
    enemyAttackTelegraphChance: 2,
    pvp: true,
    pvp_type: 'ranked',
  },
  {
    id: 'pvp_friendly_doubles',
    name: 'Friend Battle — Doubles',
    description: 'Face a friend with two Pokemon on the field at once. Four Pokemon auto set to level 50.',
    category: 'Pokemon Center',
    subCategory: 'Battles',
    icon: {type:'item',id:'vs-seeker'},
    background: '/backgrounds/friend-stadium.avif',
    requirements: [{type:'item_owned',targetId:'vs-seeker'},{type:'kid_mode',inverse:true}],
    enemyTeam: [], rewards: [], maxPokemon: 4, levelCap: 50,
    enemyAttackTelegraphChance: 2, pvp: true, pvp_type: 'friendly', format: 'double',
  },
  {
    id: 'pvp_ranked_doubles',
    name: 'Crystal League — Doubles',
    description: 'Take part in the Crystal League with two Pokemon active per side.',
    category: 'Pokemon Center',
    subCategory: 'Battles',
    icon: {type:'item',id:'vs-seeker'},
    background: '/backgrounds/crystal-stadium.avif',
    requirements: [{type:'item_owned',targetId:'vs-seeker'},{type:'kid_mode',inverse:true}],
    enemyTeam: [],
    rewards: [
      {type:'currency',targetId:'pokedollars',quantity:100},
      {type:'currency',targetId:'battle-points',quantity:5},
    ],
    maxPokemon: 4, levelCap: 20, enemyAttackTelegraphChance: 2,
    pvp: true, pvp_type: 'ranked', format: 'double',
  },
]
