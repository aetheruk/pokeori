  ...
  return (
    ...
    >
      ...
      >
        ...
        {(item.originalData as any)?.eventContexts?.map((event: any) => <p key={event.id} className="mt-1 text-xs text-game-ochre">{event.title} · Ends {new Date(event.endAt).toLocaleString()}</p>)}
        {isGrouped && (
