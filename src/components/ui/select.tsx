...
const SelectContext = React.createContext<{ id: string } | null>(null)
...
function Select({
  ...
  // useId returns a stable id for SSR and client, keeping Trigger + Content deterministic
  ...
  return (
    <SelectPrimitive.Root
      ...
      {...props}
    >
      <SelectContext.Provider value={{ id: uid }}>
        {props.children}
...
function SelectTrigger({
  ...
  // use a deterministic content id linked to the root's uid
  const contentId = ctx ? `${ctx.id}-select-content` : undefined
  return (
...
function SelectItem({
  ...
  return (
    <SelectPrimitive.Item
      ...
      className={cn(
        "relative flex min-h-10 w-full cursor-default select-none items-center gap-2 rounded-md py-2 pr-8 pl-2.5 text-sm text-game-ink outline-none focus:bg-game-moss/10 focus:text-game-moss-strong data-[disabled]:pointer-events-none data-[disabled]:opacity-50 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4 *:[span]:last:flex *:[span]:last:items-center *:[span]:last:gap-2",
        className,
      )}
      {...props}
    >
