...
import { resolveCraftRewards, shouldConsumeCraftCosts } from '@/utilities/artisan/rewards'

describe('TCG Basic Training content', () => {
  test('authors the underground task progression and visible set gates', () => {
    const basicTraining = tasks.find((task) => task.id === 'underground-tcg-basic-training')
    const redistribution = tasks.find((task) => task.id === 'underground-tcg-card-redistribution')
    const ownSet = tasks.find((task) => task.id === 'underground-tcg-my-very-own-set')
    const base4Complete = tasks.find((task) => task.id === 'underground-tcg-base4-complete')

    expect(basicTraining?.enterModal).toHaveLength(4)
    expect(basicTraining?.enterModal?.[0]?.buttons.some((button) => button.type === 'success')).toBe(false)
    ...
    expect(basicTraining?.enterModal?.[0]?.buttons.map((button) => button.text)).toEqual([
      'Gather Crystals',
      'Dig Even Deeper',
      'Profit?',
    ])
    expect(basicTraining?.enterModal?.[1]?.buttons.map((button) => button.text)).toEqual([
      'We make them ofcourse!',
      'The Pit',
    ])
    expect(basicTraining?.exitModal?.message).toContain('Gather Crystals Spread the word of the TCG')
    ...
    expect(base4Complete?.criteria).toEqual([
      { type: 'card_collected_set', targetId: 'base4', count: 130, unique: true },
    ])
  })

  test('authors the inspection, battle, and art tutorial games with one-time gates', () => {
    const inspection = allGames.find((game) => game.id === 'underground-tcg-card-memory-game')
    const fire = allGames.find((game) => game.id === 'underground-tcg-battle-fire')
    const water = allGames.find((game) => game.id === 'underground-tcg-battle-water')
    const grass = allGames.find((game) => game.id === 'underground-tcg-battle-grass')
    const artAcademy = allGames.find((game) => game.id === 'underground-tcg-art-academy')
    ...
    expect((fire!.settings as { opponentEnergyType?: string }).opponentEnergyType).toBe('Fire')
    expect((water!.settings as { opponentEnergyType?: string }).opponentEnergyType).toBe('Water')
    expect((grass!.settings as { opponentEnergyType?: string }).opponentEnergyType).toBe('Grass')
    expect(fire?.rewards).toContainEqual({ type: 'currency', targetId: 'pokedollars', quantity: 1000 })
  ...
  test('uses the modest duplicate redistribution payout table', () => {
    ...
    expect(items.find((item) => item.id === 'card-crystalizer')?.name).toBe('Card Redistribution Box')
  })

  test('authors TCG Artisan recipes with exact quality outputs', () => {
    const foil = artisanRecipes.find((recipe) => recipe.id === 'craft-tcg-foil-pack')
    const base4 = artisanRecipes.find((recipe) => recipe.id === 'craft-tcg-base4-pack')
    ...
    expect(resolveCraftRewards(base4!, 'good')[0]).toMatchObject({ targetId: 'pack-base4', quantity: 1 })
    expect(resolveCraftRewards(base4!, 'perfect')[0]).toMatchObject({ targetId: 'pack-base4', quantity: 2 })
