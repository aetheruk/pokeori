...
describe('user account security boundary', () => {
  ...
  test('registration stays public and record updates stay owner scoped', async () => {
    expect(await Users.access!.create!({ req: { user: null } } as any)).toBe(true)
  ...
  test('all authored game fields deny public and player create/update requests', async () => {
    for (const field of Users.fields) {
      if (!('name' in field) || field.type === 'join' || field.type === 'ui' || field.name === 'trainerName') continue
